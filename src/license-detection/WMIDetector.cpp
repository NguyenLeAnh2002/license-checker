...
LicenseResult WMIDetector::Detect() {
    ...
    try {
        ...
        result.SetErrorMessage(std::string("WMI detection failed: ") + ex.what());
        result.SetTimestamp(timestamp);
    }
    catch (...) {
        result.SetLicenseStatus(LicenseStatus::UnableToDetermine);
        result.SetError(true);
        result.SetErrorMessage("WMI detection failed: Unknown exception");
        result.SetTimestamp(timestamp);
    }
...
LicenseStatus WMIDetector::DetectWindowsLicenseStatus() {
...
void WMIDetector::DetectWindowsVersionAndEdition(int& outVersion, std::string& outEdition) {
    outVersion = GetWindowsVersion();
    outEdition = GetWindowsEdition();
}

// Detect Windows KMS server address
std::string WMIDetector::DetectWindowsKmsServer() {
...
LicenseStatus WMIDetector::DetectOfficeLicenseStatus() {
...
std::string WMIDetector::DetectOfficeKmsServer() {
...
std::string WMIDetector::QueryRegistryForKmsServer(const std::string& registryPath) {
    HKEY hKey = NULL;
    LONG result = RegOpenKeyExA(
        HKEY_LOCAL_MACHINE,
        registryPath.c_str(),
...
int WMIDetector::GetWindowsVersion() {
...
std::string WMIDetector::GetWindowsEdition() {
